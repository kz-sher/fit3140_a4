...
io.on('connection', function (socket) {
    ...
    socket.on('transacMade',function(data){
        ...
        validate(servers[chosen_server_num].server_no, data, socket.num);
        connected_servers_info.forEach(s=>{
           s.socket.emit('validate', chosen_server_num, data, socket.num); 
        });
    });
    ...
    socket.on('validate', function(server_num, data, socket_id){
        console.log('received validate')
        validate(server_num, data, socket_id)
    });
...
function validate(server_num, data, socket_id){
    if(server_info.server_no == server_num){
        printObjSizeInBytes(data);
